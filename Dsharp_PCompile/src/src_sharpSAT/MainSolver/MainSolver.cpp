...
void CMainSolver::getpvars(const char* lpstrFileName)
{
	char v;
	int var;
	unsigned int numpvars = 0;
	...
	///Begin File input
	ifstream input_file(lpstrFileName);
	if (!input_file) 
	{
		toERROUT("Cannot open file: " << lpstrFileName << endl);
		exit(3);
	}
	if (input_file >> v && v != 'v')
	{
		toERROUT("Invalid  Support file" << endl);
		exit(3);
	}
	while ((input_file >> var) && var != 0)
	{
		numpvars ++;
		projectingVars.insert(var); 
	}
	projectingVarCount = numpvars;
}
...
void CMainSolver::solve(const char *lpstrFileName)
{
	...
	// cout<<"Decision Level" << decStack.getDL()<<endl;
	...
	// if (CSolverConf::projectedvars)
	// {
	// 	for (int i = 0; i <= projectingVarCount; ++i)
	// 	{	
	// 		litNodes.push_back(new DTNode(i, true, num_Nodes++));
	// 		litNodes.push_back(new DTNode(-1 * i, true, num_Nodes++));
	// 	}
	// 	toSTDOUT("#Projecting Vars:" << projectingVarCount << endl);
	// }
	// else
	// {
	...
	if (decStack.top().getOverallSols() != 0)
	{
		...
		for (bbit = backbones.begin(); bbit != backbones.end(); bbit++)
		{
			...
			{
				DTNode *temp = get_lit_node(bbit->toSignedInt());
				if(CSolverConf::projectedvars){
					if(0 != projectingVars.count(getOrigTranslation()[abs(temp->getVal())]))
					{
						decStack.top().getCurrentDTNode()->addChild(temp, true);
					}
				}
				else
				{
			...
			{
				DTNode *temp = get_lit_node_full(bbit->toSignedInt());
				if(CSolverConf::projectedvars){
					if(0 != projectingVars.count(getOrigTranslation()[abs(temp->getVal())]))
					{
						decStack.top().getCurrentDTNode()->addChild(temp, true);
		...
		const CComponentId & superComp)
{
	...
	{
		isProjecting = true;
		it = superComp.varsBegin();
		while(*it != varsSENTINEL)
		{
			if( getVar(*it).isActive() && 0 != projectingVars.count(getVar(*it).getVarNum()) ){
				score = (int) getVar(*it).getVSIDSScore() + getVar(*it).getDLCSScore();
				pVV = &getVar(*it);
				break;
			}
			it++;
		}
		if(*it != varsSENTINEL && 0 != projectingVars.count(getVar(*it).getVarNum()))
		{
			score = (int) getVar(*it).getVSIDSScore() + getVar(*it).getDLCSScore();
			pVV = &getVar(*it);
			while(*it != varsSENTINEL)
	...
	// cout << "Deciding on "<< pVV->getVarNum()<< " with sign "<<pol<<endl;
	return true;
}
...
bool CMainSolver::decide()
{
	...
	newNode->decVar = deVar;
	newNode->lit = theLit.toSignedInt();
	...
	if(CSolverConf::projectedvars){
		if(0 != projectingVars.count(deVar)){
...
retStateT CMainSolver::backTrack()
{
	...
	if(!isProjecting && CSolverConf::projectedvars)
	{
		do
		{
			...
			if (decStack.top().getBranchSols() != 0.0 && isProjecting == false)
			{
				// cout << decStack.top().getOrDTNode()->decVar << endl;
				while(projectingVars.count(abs(decStack.top().getOrDTNode()->decVar)) == 0 
				&& abs(decStack.top().getOrDTNode()->decVar) !=0){
					decStack.pop();
					if (decStack.top().anotherCompProcessible())
					{
						return PROCESS_COMPONENT;
...
bool CMainSolver::BCP(vector<AntAndLit> &thePairsOfImpl)
{
	...
	for (unsigned int i = 0; i < thePairsOfImpl.size(); i++)
		if (assignVal(thePairsOfImpl[i].getLit(), thePairsOfImpl[i].getAnt()))
		{
			...
			{
				if(CSolverConf::projectedvars){
					if(0 != projectingVars.count(getVar(satLit).getVarNum()))
					{
					...
					!= SENTINEL_LIT; bt++)
			{
				if (getVar(*bt).isActive())
				{
					...
					{
						if(CSolverConf::projectedvars){
							if(0 != projectingVars.count(getVar(*bt).getVarNum()))
							{
			...
			for (it = pUnWatchCls->end() - 1; *it != SENTINEL_CL; it--)
			{
				...
				if (getVar(pCl->idLitA()).isActive())
				{
					if (!getVar(pCl->idLitB()).isActive()) //IMPLIES_LITA;
					{
						...
						{
							if(CSolverConf::projectedvars){
								if(0 != projectingVars.count(getVar(pCl->idLitA()).getVarNum()))
								{
				...
				else
				{
					...
					{ //IMPLIES_LITB;
						...
						{
							if(CSolverConf::projectedvars){
								if(0 != projectingVars.count(getVar(pCl->idLitB()).getVarNum()))
								{
									DTNode * ccLit = get_lit_node(
		...
		for (jt = nextStep.begin(); jt != nextStep.end(); jt++)
			if (getVar(*jt).isActive())
			{
				...
				if (!bSucceeded)
				{
					...
					if(CSolverConf::projectedvars){
						if(0 != projectingVars.count(getVar(theLit).getVarNum()))
						{
							...
							DTNode * ibcpLit = new DTNode(DT_TOP, num_Nodes++);
							ibcpLit->addParent(decStack.top().getCurrentDTNode(), true);
							implPairs.clear();
						}
					}
