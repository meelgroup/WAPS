...
public:
	...
	DTNode * get_lit_node_full(int lit)
	{
		for (int i = 0; i < litNodes.size(); i++) {
			if (lit == litNodes[i]->getVal()) {
				// cout<<"lit node "<<litNodes[i]<<endl;
				return litNodes[i];
			}
		}
		litNodes.push_back(new DTNode(lit, true, num_Nodes++));
		// cout<<"lit node "<<litNodes[litNodes.size() - 1]->getVal()<<endl;
	...
	void getpvars(const char *lpstrFileName);
