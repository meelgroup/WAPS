...
int main(int argc, char *argv[])
{
	...
	if (argc <= 1)
	{
		...
		cout << "\t -pvarsfile [file]\t file containing the variables on which projected compilation has to be done" << endl;
	...
	for (int i = 1; i < argc; i++)
	{
		...
		else if (strcmp(argv[i], "-pvarsfile") == 0)
		{
			if (argc <= i + 1)
			{
			...
			CSolverConf::projectedvars = true;
			memset(pvarsFile, 0, 1024);
			strcpy(pvarsFile, argv[i + 1]);
		}
